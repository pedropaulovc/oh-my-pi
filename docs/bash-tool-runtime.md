...
1. **Tool-call surface** (`toolName: "bash"`): used when the model calls the bash tool.
   ...
   - Finite-command parameters include `command`, optional `timeout`, `cwd`, `pty`, and, when `async.enabled` is true, `async` (`true` or `"auto"`) plus `progress` (`"wake"` or `"ambient"`). A `name` selects supervised service mode with optional `ready`, `env`, and `progress` (`"wake"`, `"ambient"`, or `"off"`), but no `async` or `timeout`.
...
- rejects finite-command `progress` unless the same call uses `async: true` or `async: "auto"`; finite commands reject `off`, while named services accept it,
- defaults `timeout` to 300 seconds; `0` explicitly disables the command deadline.

A named service takes the launch-broker path instead of the finite-command pipeline below. Reusing a live name restarts it with the new spec. Read `proc://<name>` for status/output; write stdin to that URL, `wake`/`ambient`/`off` to `proc://<name>/progress`, or omit content at `proc://<name>/kill` to stop it. `ready` waits for all supplied log/port conditions before returning. Monitoring and process lifetime are independent.
...
While inline, the manager keeps a foreground-backed job hidden and suppresses background progress/completion. Promotion uses `backgroundJob`; inline completion or abort releases the foreground job with `releaseForegroundJob`. At-cap and short-deadline direct-inline paths do not register a background candidate.

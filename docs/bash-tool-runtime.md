...
Rate limiting drops complete post-batch events by timing, not severity. Suppression counts events rather than lines and include none of the omitted text, so delivered progress cannot prove that an error or transition did not occur. The full artifact is authoritative. Buckets are per job or monitor; aggregate wake traffic grows across concurrent sources.

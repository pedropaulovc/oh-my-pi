...
| `async.progress.maxUpdates`      | number  | `200`     | Cap on progress updates per job; each is a persisted message, so an unbounded monitor grows the transcript for the session. The job keeps running and its result is still delivered. `0` disables the cap. |

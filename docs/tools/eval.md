...
- The tool foreground-waits for `resolveAutoBackgroundWaitMs(thresholdMs, clampedCellTimeoutMs, "runtime")`: the threshold, clamped down to the cell's own clamped timeout minus a 1 s buffer so a deadline expiry resolves inline rather than backgrounding moments before it fires. The cell budget is runtime (it pauses across `agent()`/tool-bridge waits), so unlike Bash's wall-clock deadline a short budget never forces inline-only execution. Raising `timeout` therefore does not extend foreground execution beyond the threshold. A threshold of `0` backgrounds immediately.
- The manager tracks the foreground-backed execution without exposing it as a background job or delivering progress/completion. Promotion calls `backgroundJob`; inline completion or abort calls `releaseForegroundJob` instead.
- On backgrounding, the tool returns the live output tail plus `Backgrounded as job <id> (<cell label>); result will be delivered automatically.`, with `details.async = { state: "running", jobId, type: "eval" }`. The label is the cell title or its first code line, collapsed to one line (max 120 chars). The job's completion is delivered later like a backgrounded bash command.
...
- Inspect jobs with `read proc://<id>` and cancel with `write proc://<id>/kill` (no content required). Results auto-deliver; call `wait` only when blocked with no other work, never to keep a turn alive for wake progress.

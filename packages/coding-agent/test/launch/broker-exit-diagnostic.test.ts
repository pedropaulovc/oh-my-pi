...
describe("daemon broker exit diagnostics", () => {
	...
	it("preserves a recovered terminal diagnostic without waiting on a replacement", async () => {
		...
		const broker = startBroker(projectDir, runtimeDir);
		try {
			// Upstream returns terminal pattern waits immediately, rather than
			// waiting for a concurrent restart to reject the old generation.
			...
			if (recovered.op !== "wait") throw new Error("unexpected wait result");
			expect(recovered.timedOut).toBe(false);
			expect(recovered.matched).toBeUndefined();
			expect(recovered.daemon.exitCode).toBe(58);
			const diagnostic = serviceStatus(recovered.daemon);
			expect(diagnostic).toContain("Reason: ~/private/recovered");
			expect(diagnostic).not.toContain(os.homedir());
			expect(diagnostic).not.toContain("\u001b");
			const restarted = await client.request({ op: "restart", name: "code-58" });
			if (restarted.op !== "restart") throw new Error("unexpected restart result");
			expect(restarted.daemon.id).not.toBe(recovered.daemon.id);
			const replacement = await client.request({ op: "wait", name: "code-58", for: "exit", timeoutMs: 5_000 });
			if (replacement.op !== "wait") throw new Error("unexpected replacement wait result");
			expect(replacement.daemon.exitReason).toBe(
				"process exited with code 58 without a reported termination reason",
			);
		} finally {
			await shutdown(client, broker);
			process.title = previousTitle;

...
import { findBackgroundNotice, formatBackgroundNotice } from "@oh-my-pi/pi-tui/tools/bash";
...
describe("background notice", () => {
	test("names the job's command so parallel results stay attributable out of order", () => {
		...
		expect(notice).toBe(
			"Backgrounded as job bg_5 (uv run verify.py); its output is injected into the conversation as a follow-up the moment it finishes. Do NOT poll for it (no `sleep`, `ps`, `pgrep`, `top`, `pidwait`, log tailing): every poll is a wasted turn. Do other work, or end your reply and wait to be woken.",
		);
	});

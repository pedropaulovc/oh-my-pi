...
function makeSession(manager: AsyncJobManager, overrides: Record<string, unknown> = {}): ToolSession {
	const settings = Settings.isolated({ ...SETTINGS, ...overrides });
	return {
		cwd: "/tmp",
		hasUI: false,
		skills: [],
...
function withShellEnvironment(values: Record<string, string>, command: string): string {
	const exports = Object.entries(values).map(([name, value]) => `export ${name}=${shellEscape(value)}`);
...
describe("bash progress parameter", () => {
	...
	test("runs auto inline with a notice at the running-job cap while explicit async errors", async () => {
		...
		await tool.execute("occupy-slot", {
			command: withShellEnvironment(
				{ RELEASE: releasePath },
				'while [ ! -f "$RELEASE" ]; do sleep 0.01; done; printf "slot-released\\n"',
			),
			async: true,
		});
	...
	test("promotes past a stalled pre-promotion delivery and keeps the terminal text", async () => {
		...
		const result = await tool.execute("stalled-auto-promote", {
			command: withShellEnvironment(
				{ RELEASE: releasePath },
				'export STALL_LINE="$(printf stalled)-line"; export AFTER_LINE=after-stall; ' +
					'printf "$STALL_LINE\\n"; ' +
		...
		const execution = tool
			.execute(
				"drain-abort",
				{
					...
					command: withShellEnvironment(
						{ RELEASE: releasePath, PIDFILE: pidPath },
						"printf 'stalled-line\\n'; " +
	...
	test("freezes the foreground preview before a mirror-delayed post-boundary callback", async () => {
		...
		const backgrounded = Promise.withResolvers<void>();
		let postBoundaryCallbackRan = false;
		const backgroundJob = manager.backgroundJob.bind(manager);
		manager.backgroundJob = jobId => {
			const promoted = backgroundJob(jobId);
		...
		manager.activateProgressDelivery = (jobId, delivery, foregroundStreamProvenance, coverage) => {
			...
			queueMicrotask(() => {
				...
				void backgrounded.promise.then(() => reportAgentProgress?.("post-boundary"));
			});
			return activated;
		};
	...
	test("drains a throttle-held chunk into the preview without replaying it after promotion", async () => {
		...
		const execution = tool.execute(
			...
			{
				command: withShellEnvironment(
					{ GATE: gatePath, RELEASE: releasePath },
					"printf 'inline-first\\n'; " +

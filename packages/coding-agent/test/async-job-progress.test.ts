...
describe("AsyncJobManager model progress", () => {
	...
	test("holds foreground-backed progress until the job is backgrounded", async () => {
		vi.useFakeTimers();
		const manager = new AsyncJobManager({});
		const mine = recordingSink();
		manager.registerProgressSink("Main", mine.sink);
		...
		const jobId = manager.register(
			...
			{ ownerId: "Main", progressDelivery: "ambient", foreground: true },
		);
		const report = await started.promise;
		...
		expect(manager.backgroundJob(jobId)).toBe(true);
		report("backgrounded");
		vi.advanceTimersByTime(200);
		expect(mine.seen.map(item => item.text)).toEqual(["backgrounded"]);

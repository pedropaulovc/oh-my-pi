...
- `progress` (async only) reports while the command runs: `every` (seconds) for a heartbeat, `match` (regex) to fire on a line, `stopOnMatch` to end the command there, `wake: true` to be told even while idle. Retune or stop it later with `hub` `{"op":"monitor"}`.{{/if}}
</instruction>

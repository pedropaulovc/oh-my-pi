...
import type { AsyncJob, AsyncJobDetails, AsyncJobManager, AsyncJobProgressDelivery, AsyncJobType } from "./job-manager";
...
/**
 * `proc://<job-id>/progress`: flip each running job's progress delivery mode in
 * place. Unlike cancellation this never consumes results — a retune is not a
 * result recovery, so a settled job's text stays recoverable by a later `wait`
 * or `proc://` read.
 */
